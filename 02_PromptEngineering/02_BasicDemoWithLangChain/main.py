...
def get_session_history(user_id: str, conversation_id: str):
    ...
    for i, message in enumerate(recent_messages):
        # 根据索引来决定角色，偶数为user，奇数为assistant
        role = "user" if i % 2 == 0 else "assistant"
        # 创建新的ChatMessage对象
        new_message = ChatMessage(content=message.content, role=role)
        new_messages.append(new_message)
    # 将新消息添加到历史中
    for message in new_messages:
        history.add_message(message)
    logger.info(f"历史对话记忆内容: \n{history}\n")
    return history
...
async def lifespan(app: FastAPI):
    ...
    global chat_model, prompt, chain, with_message_history
    try:
        ...
        # （3）定义chain
        chain = prompt | getPrompt | chat_model | StrOutputParser()
...
async def chat_completions(request: ChatCompletionRequest):
    # 申明引用全局变量，在函数中被初始化，并在整个应用中使用
    if not chat_model or not prompt or not chain:
        logger.error("服务未初始化")
    ...
    try:
        ...
        #         results = re.findall(r'(Y|N)', formatted_response)
        #         # 统计Y和 N的数量
